...
class ResnetConformer_sed_doa_nopool(nn.Module):
    ...
    def forward(self, x):
        ...
        sed = self.sed_out_layer(outputs)
        dist = self.dist_out_layer(outputs)
        pred = torch.cat((dist, sed), dim=-1) # [32, 100, 65]
        return pred

if __name__ == '__main__':
# 参数定义
    batch_size = 4
    in_channel = 10       # 比如7通道麦克风
    time_steps = 600     # 输入时间帧数
    freq_bins = 64       # 频率维度
    out_dim = 13 * 3     # 每类事件3个DOA坐标
    in_dim = 64  # 用于计算embedding维度
    ...
    # 构造输入数据
    x = torch.randn(batch_size, in_channel, time_steps, freq_bins)
    ...
    print("输出 shape:", output.shape)  # 期望 [batch_size, T', 65]