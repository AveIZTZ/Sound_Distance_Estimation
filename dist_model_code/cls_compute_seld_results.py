...
############## Permutation 3 ####################
def compute_dist_metrics_extended_perm3(params, ref_files_folder=None):
    '''
    ...
    def get_all_metrics(true_dists, out_dists, gt_masks, pred_masks):
        '''
    ...
    def agg_metrics(this_metrics):
        ...
        glob_gt_dist.extend(this_metrics['gt_dist'])
        glob_out_dist.extend(this_metrics['out_dist'])
        glob_gt_mask.extend(this_metrics['gt_mask'])
        glob_pred_mask.extend(this_metrics['pred_mask'])
    ...
    data_wise_metrics = {'gt_dist': [], 'out_dist': [],'gt_mask': [], 'pred_mask': []}

    for gt_file in os.listdir(gt_sup_dir):

        output_file = os.path.join(ref_files_folder, gt_file)
        gt_dict = feat_cls.load_output_format_file(os.path.join(gt_sup_dir, gt_file))
        try:
            out_dict = feat_cls.load_output_format_file(output_file)
        except:
            continue
        ...
        for frame in range(nb_ref_frames + 1):
            gt_dist, gt_mask = 0, 0
            if frame in gt_dict:
                gt_dist = gt_dict[frame][0][1]
                gt_mask = 1

            out_dist = out_dict[frame][0][0]
            pred_mask = out_dict[frame][0][1]

            data_wise_metrics['gt_dist'].append(gt_dist)
            data_wise_metrics['out_dist'].append(out_dist)
            data_wise_metrics['gt_mask'].append(gt_mask)
            data_wise_metrics['pred_mask'].append(pred_mask)
...
def compute_dist_metrics_perm3(params, ref_files_folder=None):
    '''
    ...
    def agg_metrics(this_metrics):
        ...
        glob_gt_mae.extend(this_metrics['gt_mae'])
        glob_gt_mpe.extend(this_metrics['gt_mpe'])
        glob_overall_mpe.extend(this_metrics['overall_mae'])
    ...
    data_wise_metrics = {}
    data_wise_metrics['gt_mae'] = []
    data_wise_metrics['gt_mpe'] = []
    data_wise_metrics['overall_mae'] = []
    ...
    for gt_file in os.listdir(gt_sup_dir):
        ...
        for frame in range(nb_ref_frames + 1):
            ...
            if gt_mask == 1:
                data_wise_metrics['gt_mae'].append(this_gt_mae)
                data_wise_metrics['gt_mpe'].append(this_gt_mpe)
            data_wise_metrics['overall_mae'].append(this_over_mae)
